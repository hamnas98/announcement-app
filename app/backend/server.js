  ...
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      return res.status(401).json({
        ...
        hint: "Frontend must use useAuthenticatedFetch from App Bridge",
      });
    }
    ...
    if (!session) {
      return res.status(401).json({
        ...
        shop: shopDomain,
        fix: `Visit /api/auth?shop=${shopDomain} to reinstall`,
      });
    }

    res.locals.shopify = { session };
    console.log(`🔓 Verified session for: ${shopDomain}`);
    next();
  } catch (error) {
